...
class Listings{
    ...
    public function generateForm($area=0,$town=0,$city=0,$state=0){
        $this->getListTypes();
    ...
    public function getAdmStates(){
      $this->AdmStates = array();
      $getStates = new SqlIt("
      SELECT l.*, COUNT(p.property_id) AS props
      FROM locations_states AS l
      LEFT JOIN property_list AS p ON l.state_id = p.state
      GROUP BY state_id ORDER BY state ASC","select",array());
      if($getStates->NumResults > 0){
        foreach($getStates->Response as $gs){
           $this->AdmStates[$gs->state_id]['name'] = $gs->state;
           $this->AdmStates[$gs->state_id]['cnt'] = $gs->props;
           $this->AdmStates[$gs->state_id]['featured'] = 0; // States don't have featured flag
        }
      }
    }
            ...
            if($addNew){
                ...
                if($post['display_status_es'] != '' || $post['title_es'] != ''){
                  ...
                  $add = new SqlIt("
                    ...
                    if(isset($post['feature_name_es'][0])){
                        foreach($post['feature_name_es'] as $key=>$val){
                            if($post['feature_name_es'][$key] != '' && $post['feature_value_es'][$key] != ''){
