 ...
 */
 function sendfile(file, editor, welEditable) {
   ...
   $.ajax({
     data: data,
     type: "POST",
     url: "assets/inc/process/upload_blog.php",
     cache: false,
     contentType: false,
     processData: false,
     dataType: 'json',
     success: function(url) {
       if(url.success === 1){
         ...
         var imgurl = 'uploads/blog/'+url.filename;
         //$('.summernote').summernote('insertImage', welEditable, imgurl);
         var image = $('<img>').attr('src', imgurl);
         $(editor).summernote("insertNode", image[0]);
         //console.log(imgurl);
       }else if(url.error === 1){
         alert('There was an error uploading this image. Please check image size and format.');
       }
     }
   });
 }
        ...
        var b = {
          ...
          callbacks: {
            onImageUpload: function(files, welEditable) {
              ...
              //b.summernote('insertNode', imgNode);
              console.log('yeah');
  ...
  function(a) {
    provider.initUIs = function() {
      ...
      provider.initFullscreen = function() {
          ...
          if (!a("#sidebar-collapse-styles").length) {
            a('<style id="sidebar-collapse-styles">').text('.sidebar-collapsed .sidebar{width:0!important;transform:translateX(-100%);transition:all .3s ease}.sidebar-collapsed .topbar,.sidebar-collapsed .main-container{margin-left:0!important;transition:all .3s ease}.sidebar-collapsed .hamburger-menu{display:inline-block!important}.hamburger-menu{display:none;margin-right:10px;font-size:18px;cursor:pointer;padding:8px;border-radius:3px;background:rgba(255,255,255,0.1);color:inherit}.hamburger-menu:hover{background:rgba(255,255,255,0.2)}').appendTo("head")
          }
          a(b).each(function() {
            var $button = a(this);
            var $hamburger = $button.parent().find(".hamburger-menu");
            if (!$hamburger.length) {
              $hamburger = a('<span class="hamburger-menu">☰</span>');
              $button.before($hamburger);
              $hamburger.on("click", function() {
                a("body").removeClass("sidebar-collapsed");
                updateIcon()
              })
            }
          });
          function updateIcon() {
            var isCollapsed = a("body").hasClass("sidebar-collapsed");
            a(b).each(function() {
              var $this = a(this);
          ...
          updateIcon();
          a(document).on("click", b, function(e) {
            e.preventDefault();
            var isCollapsed = a("body").hasClass("sidebar-collapsed");
            if (isCollapsed) {
              a("body").removeClass("sidebar-collapsed")
            } else {
              a("body").addClass("sidebar-collapsed")
            }
            updateIcon()
          })
