...
app.ready(function () {
  ...
  $(".select2").select2({
    ...
    // Get and display cities when state changes
    $("#nj_states").on('change',function(){
        var stateId = $(this).val();
        $("#nj_city").html('<option><i class="fas fa-spinner fa-spin"></i> Loading...</option>');
        $("#nj_city").addClass('loading-select').attr('disabled','disabled');
        $("#nj_county").html('<option></option>');
        $("#nj_county").attr('disabled','disabled');
        $("#nj_areas").html('<option></option>');
        $("#nj_areas").attr('disabled','disabled');
        $.post('assets/inc/process/get_areas.php',{states:stateId},function(data){
            $("#nj_city").html(data);
            $("#nj_city").removeClass('loading-select').removeAttr('disabled');
        });
    });
    ...
    $("#add_feature").on('click', function (e) {
        ...
        var newIconSelect = iconSelect.clone();
        newIconSelect.val(selectedIcon);
        
        // Create the new row
        var newtr = $('<tr class="bg-pale-success sort-it"><td></td><td><input type="text" name="feature_name[]" class="form-control" placeholder="Feature name" value="' + fname.val() + '"></td><td><input type="text" name="feature_value[]" class="form-control" placeholder="Feature value" value="' + fval.val() + '"></td><td><button type="button" class="btn btn-pure btn-danger delete-tr" data-provide="tooltip" title="Delete this feature"><i class="ti-trash"></i></button></td><td><i class="ti-move"></i></td></tr>');

        // Insert the select into the first td
        newtr.find('td:first').append(newIconSelect);
        
        newtr.insertAfter('#add_new_feature');
        
        // Initialize selectpicker on the new dropdown with the selected value
        newIconSelect.selectpicker({
            dropupAuto: false,
            size: 8,
            liveSearch: true,
            noneSelectedText: 'Select Icon',
        ...
        newIconSelect.selectpicker('val', selectedIcon);
    ...
    $('body').on('click',"#add_feature_es", function (e) {
        ...
        // Create the new row
        var newtr = $('<tr class="bg-pale-success"><td></td><td><input type="text" name="feature_name_es[]" class="form-control" placeholder="Feature name" value="' + fname.val() + '"></td><td><input type="text" name="feature_value_es[]" class="form-control" placeholder="Feature value" value="' + fval.val() + '"></td><td><button type="button" class="btn btn-pure btn-danger delete-tr" data-provide="tooltip" title="Delete this feature"><i class="ti-trash"></i></button></td><td><i class="ti-move"></i></td></tr>');
