...
if(isset($_POST['dd']) && isset($_POST['dtype'])){
    ...
    if($dd === false || empty($dtype)) {
        ...
        $allowed_tables = ['locations_states', 'locations_cities', 'locations_towns', 'locations_areas'];
        $allowed_columns = ['state_id', 'city_id', 'town_id', 'area_id', 'state', 'location', 'town_name', 'area_name'];
...
if(isset($_POST['add_location']) && isset($_POST['dtype'])){
    ...
    if($db['tb'] != ''){
      ...
      if($_POST['dtype'] == 'state'){
        // States don't have a parent
        new SqlIt("INSERT INTO ".$db['tb']." (".$db['fi'].") VALUES (?)","insert",array($_POST['dname']));
      } else {
      ...
      $show_alert = 'success';
      $note_head = 'Success';
      $note_txt = 'The '.$_POST['dtype'].' has been added successfully.';
    }
}

if($dtype == 'state'){
  $locations->getAdmStates();
  $loc_list = $locations->AdmStates;
  $reassign = array();
  $page_desc = 'All States';
}
                          ...
                          <div class="card-body">
                              ...
                              <ol class="how-to-locations">
                                ...
                                <li>
                                  ...
                                  <?php if($dtype == 'city'){ ?>
                                    <a href="property_locations.php" class="d-block link mt-1"><i class="ti-arrow-left"></i> Return to list of states</a>
                                  <?php }else{ ?>
                            ...
                            <table class="table table-striped" id="tb_locs">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th><?= ucfirst($dtype) ?></th>
                                        <th>Properties</th>
                                        <?php if($dtype != 'state'){ ?><th>Menu</th><?php }else{ echo '<th colspan="2"></th>'; } ?>
